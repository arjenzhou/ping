...
public class ConnectorLoader<T> {
    private static final String FOLDER = "META-INF/ping/";
    private static final Map<Class<?>, ConnectorLoader<?>> LOADERS = new ConcurrentHashMap<>();
    private final Map<String, Class<T>> extensions = new ConcurrentHashMap<>();
    private Class<T> service;

    public static <T> ConnectorLoader<T> getConnectorLoader(Class<T> service) {
        if (service == null) {
        ...
        ConnectorLoader<T> loader = (ConnectorLoader<T>) LOADERS.get(service);
        if (loader == null) {
            LOADERS.putIfAbsent(service, new ConnectorLoader<T>());
            loader = (ConnectorLoader<T>) LOADERS.get(service);
    ...
    private Map.Entry<String, String> parseLine(String line) {
        ...
        if (line.length() > 0) {
            ...
            if (split.length == 2) {
                ...
                return new AbstractMap.SimpleEntry<>(extensionType, extensionName);
            }
        }
        return null;
    }
